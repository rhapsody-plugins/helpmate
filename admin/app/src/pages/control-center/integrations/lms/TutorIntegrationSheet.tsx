    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="flex flex-col w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Tutor LMS</SheetTitle>
          </SheetHeader>

          <div className="flex flex-col gap-4 p-4">
            {!tutorPluginActive ? (
              <p className="text-sm text-muted-foreground">
                Install and activate Tutor LMS to enable LMS integration.
              </p>
            ) : !statusQuery.data?.active ? (
              <p className="text-sm text-muted-foreground">
                Tutor plugin is active but runtime is not fully loaded yet. Refresh after Tutor
                initializes.
              </p>
            ) : (
              <>
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                One-way import from Tutor LMS: students are matched by email and contact profile
                fields are overwritten with Tutor user data.
              </p>
            </div>
          </div>
      ...
      <Dialog open={syncConfirmOpen} onOpenChange={setSyncConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Sync Tutor LMS students to CRM?</DialogTitle>
            <DialogDescription className="space-y-2 text-left">
              <span className="block">
                Existing CRM contacts with the <strong>same email</strong> as a Tutor LMS student
