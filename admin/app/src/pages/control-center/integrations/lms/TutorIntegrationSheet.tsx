...
type TutorIntegrationSheetProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  tutorPluginActive: boolean;
};

export default function TutorIntegrationSheet({
  open,
  onOpenChange,
  tutorPluginActive,
}: TutorIntegrationSheetProps) {
  const queryClient = useQueryClient();
  const [syncConfirmOpen, setSyncConfirmOpen] = useState(false);

  const statusQuery = useQuery<TutorStatus>({
    queryKey: ['integrations-tutor-status'],
    queryFn: async () => {
    ...
    enabled: open && tutorPluginActive,
  ...
  const syncMutation = useMutation({
    mutationFn: async () => {
      const response = await api.post<{
        ...
        summary?: SyncSummary;
        message?: string;
      }>('/integrations/tutor/sync-students');
  ...
  const canSync = tutorPluginActive && statusQuery.data?.active === true && !syncMutation.isPending;
    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="flex flex-col w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{__('Tutor LMS')}</SheetTitle>
          </SheetHeader>

          <div className="flex flex-col gap-4 p-4">
            {!tutorPluginActive ? (
              <p className="text-sm text-muted-foreground">
                {__('Install and activate Tutor LMS to enable LMS integration.')}
              </p>
            ) : !statusQuery.data?.active ? (
              <p className="text-sm text-muted-foreground">
                {__(
                  'Tutor plugin is active but runtime is not fully loaded yet. Refresh after Tutor initializes.'
                )}
              </p>
            ) : (
              <>
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {__(
                  'One-way import from Tutor LMS: students are matched by email and contact profile fields are overwritten with Tutor user data.'
                )}
              </p>
            </div>
          </div>
      ...
      <Dialog open={syncConfirmOpen} onOpenChange={setSyncConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{__('Sync Tutor LMS students to CRM?')}</DialogTitle>
            <DialogDescription className="space-y-2 text-left">
              <span className="block">
                {__(
                  'Existing CRM contacts with the same email as a Tutor LMS student will be updated from that student profile.'
                )}
              </span>
