    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="flex flex-col w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{__('Ultimate Member')}</SheetTitle>
          </SheetHeader>

          <div className="flex flex-col gap-4 p-4">
            {!ultimateMemberPluginActive ? (
              <p className="text-sm text-muted-foreground">
                {__('Install and activate Ultimate Member to enable this integration.')}
              </p>
            ) : !statusQuery.data?.active ? (
              <p className="text-sm text-muted-foreground">
                {__(
                  'Ultimate Member is active but runtime is not fully loaded yet. Refresh after it initializes.'
                )}
              </p>
            ) : (
              <>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between rounded-md border p-3">
                    <div className="space-y-1">
                      <Label htmlFor="um-integration-enabled">{__('Enable UM integration events')}</Label>
                      <p className="text-xs text-muted-foreground">
                        {__(
                          'When enabled, Helpmate records UM lifecycle events and syncs member profile data to CRM.'
                        )}
                      </p>
                    </div>
                    <Switch
                ...
                <div className="border-t pt-4 space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {__(
                      'One-way import from Ultimate Member. Contacts are matched by email and profile fields may be overwritten by UM values.'
                    )}
                  </p>
                </div>
      ...
      <Dialog open={syncConfirmOpen} onOpenChange={setSyncConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{__('Sync Ultimate Member members to CRM?')}</DialogTitle>
            <DialogDescription className="space-y-2 text-left">
              <span className="block">
                {__(
                  'Existing CRM contacts with the same email as a UM member can be updated from that member profile.'
                )}
              </span>
