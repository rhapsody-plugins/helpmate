...
class Helpmate_Ninja_Forms_Integration
{
	/**
	 * @var Helpmate
	 */
	...
	/**
	 * Pending sanitized order payload keyed by Ninja form id.
	 *
	...
	public function __construct($helpmate, $events)
	{
		...
		add_filter('ninja_forms_submit_data', [$this, 'handle_submission'], 20, 1);
		add_filter('ninja_forms_post_run_action_type_successmessage', [$this, 'filter_success_message_action_data'], 20, 1);
		add_action('wp_enqueue_scripts', [$this, 'enqueue_ninja_forms_order_status_script'], 1000);
		add_action('wp_footer', [$this, 'maybe_enqueue_ninja_forms_order_status_in_footer'], 5);
	}
	...
	public function get_admin_forms_payload()
	{
		...
		$saved_settings = $this->helpmate->get_settings()->get_setting('ninja_forms_integrations', []);
		$saved_forms = isset($saved_settings['forms']) && is_array($saved_settings['forms']) ? $saved_settings['forms'] : [];
		$ninja_forms = Ninja_Forms()->form()->get_forms();

		$forms = [];
		foreach ((array) $ninja_forms as $form_model) {
			if (!is_object($form_model) || !method_exists($form_model, 'get_id')) {
				continue;
			}

			$form_id = absint($form_model->get_id());
			if ($form_id <= 0) {
				continue;
			}

			$title_raw = method_exists($form_model, 'get_setting') ? $form_model->get_setting('title') : '';
			$saved_form = isset($saved_forms[$form_id]) && is_array($saved_forms[$form_id]) ? $saved_forms[$form_id] : [];
			$forms[] = [
				'id' => $form_id,
				'title' => sanitize_text_field($title_raw ?: ('Form #' . $form_id)),
	...
	/**
	 * Ninja submit interception and validation.
	 *
	...
	public function handle_submission($form_data)
	{
		...
		$form_id_fallback = isset($form_data['id']) ? absint($form_data['id']) : 0;
		if ($form_id <= 0 && $form_id_fallback > 0) {
			$form_id = $form_id_fallback;
		}
		if ($form_id <= 0) {
			return $form_data;
		}
		...
		$posted_map = $this->normalize_ninja_fields(isset($form_data['fields']) ? $form_data['fields'] : []);
		...
		if (!empty($required_missing)) {
			...
			$mapped_field_ids = $this->resolve_mapped_field_ids(
				$required_missing,
				$field_map,
				isset($form_data['fields']) ? $form_data['fields'] : []
			);
			$with_errors = $this->append_form_error($form_data, $user_message, $mapped_field_ids);
		...
		if ($this->is_pro_action($action) && !$this->is_pro_available()) {
			...
			return $this->append_form_error($form_data, $message);
		}

		// Skip dedup for order status checks: repeat lookups for same order id are expected.
		if ($action !== 'order_status_checking' && $this->events->is_duplicate('ninja_forms', $action, $context['dedup_key'], 300)) {
			...
			return $this->append_form_error($form_data, $dup_message);
		}
		...
		try {
			...
			if ($response instanceof WP_Error) {
				...
				return $this->append_form_error($form_data, $response->get_error_message());
			}
			...
			return $this->append_form_error($form_data, __('Submission failed due to an integration error.', 'helpmate-ai-chatbot'));
		}
	...
	/**
	 * Inject hidden payload + fallback card into Ninja success action response.
	 *
	 * @param array<string, mixed> $data
	 * @return array<string, mixed>
	 */
	public function filter_success_message_action_data($data)
	{
		...
		$container = sprintf(
			'<div class="helpmate-ninja-forms-order-status-payload" data-helpmate-order-status="%1$s" data-helpmate-order-status-form-id="%2$d"></div>',
		...
		/**
		 * Ninja success message when Helpmate order-status card exists.
		 *
		 * @param string               $message Default empty.
		 * @param array<string,mixed>  $order   Sanitized order payload.
		 * @param int                  $form_id Ninja form id.
		 * @param array<string,mixed>  $data    Current Ninja response data.
		 */
		$message = (string) apply_filters('helpmate_ninja_forms_order_status_response_message', '', $order, $form_id, $data);

		if (!isset($data['actions']) || !is_array($data['actions'])) {
			$data['actions'] = [];
		}

		$data['actions']['success_message'] = $message . $container . $card_html;
		$data['helpmate_order_status'] = $order;
		$data['helpmate_order_status_form_id'] = $form_id;

		return $data;
	}

	public function enqueue_ninja_forms_order_status_script()
	{
		if (is_admin()) {
			return;
		}
		...
		if (!wp_script_is('nf-front-end', 'registered')) {
			return;
		}
		...
		$load = (bool) apply_filters('helpmate_ninja_forms_order_status_enqueue_script', true);
		if (!$load) {
			return;
		}
		...
		wp_enqueue_script(
			'helpmate-ninja-forms-order-status',
			HELPMATE_URL . 'public/js/helpmate-ninja-forms-order-status.js',
			['jquery', 'nf-front-end'],
			HELPMATE_VERSION,
			true
		);
	...
	public function maybe_enqueue_ninja_forms_order_status_in_footer()
	{
		if (is_admin() || wp_script_is('helpmate-ninja-forms-order-status', 'enqueued')) {
			return;
		}
		if (!wp_script_is('nf-front-end', 'registered')) {
			return;
		}
		$this->enqueue_ninja_forms_order_status_script();
	}
	...
	{
		$settings = $this->helpmate->get_settings()->get_setting('ninja_forms_integrations', []);
	 ...
	 * @return array<string, mixed>
	 */
	private function normalize_ninja_fields($fields)
	{
		$out = [];
		foreach ((array) $fields as $field) {
			...
			$out[$key] = isset($field['value']) ? $field['value'] : null;
		}
		return $out;
	}
	...
	private function dispatch_action($action, array $payload, array $context)
	{
		...
		if ($this->is_pro_action($action)) {
			$result = apply_filters('helpmate_ninja_forms_pro_dispatch', null, $action, $payload, $context);
			if ($result === null) {
				return new WP_Error('ninja_forms_pro_handler_missing', __('Pro integration handler is not available.', 'helpmate-ai-chatbot'));
			}
			return $result;
		}
		return new WP_Error('ninja_forms_unknown_action', __('Unknown integration action.', 'helpmate-ai-chatbot'));
	}
	 ...
	 * @param array<int, int> $field_ids
	 * @return array<string, mixed>
	 */
	private function append_form_error(array $form_data, $message, array $field_ids = [])
	{
		...
		if (!isset($form_data['errors']['form']) || !is_array($form_data['errors']['form'])) {
			$form_data['errors']['form'] = [];
		}
		$form_data['errors']['form'][] = $sanitized;

		// Ninja Submission controller checks _form_data['errors']['fields'][id] during field loop.
		if (!empty($field_ids)) {
			...
			foreach ($field_ids as $field_id) {
				...
				$form_data['errors']['fields'][$field_id] = [
					'slug' => 'helpmate_required',
					'message' => $sanitized,
				];
			}
		}
	...
	/**
	 * Resolve Ninja field ids for missing mapped targets so submit gets blocked reliably.
	 *
	 * @param array<int, string> $missing_targets
	 * @param array<string, string> $field_map
	 * @param array<int, mixed> $submitted_fields
	 * @return array<int, int>
	 */
	private function resolve_mapped_field_ids(array $missing_targets, array $field_map, $submitted_fields)
	{
		$source_keys = [];
		...
		$ids = [];
		foreach ((array) $submitted_fields as $field) {
			if (!is_array($field)) {
				continue;
			}
	...
	private function render_order_status_card_html(array $order, $form_id)
	{
		...
		?>
		<div
			class="helpmate-ninja-forms-order-card helpmate-cf7-order-status"
	...
	private function extract_form_fields($form_id)
	{
		...
		$fields = [];
		foreach ((array) $form_fields as $field_model) {
			if (!is_object($field_model) || !method_exists($field_model, 'get_setting')) {
				continue;
			}
			$key = sanitize_text_field((string) $field_model->get_setting('key'));
			if ($key === '') {
				continue;
			}
			$type = sanitize_text_field((string) $field_model->get_setting('type'));
			$fields[] = [
				'name' => $key,
				'type' => $type,
			];
