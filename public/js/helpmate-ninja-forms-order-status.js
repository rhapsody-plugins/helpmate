/**
 * Render Helpmate order details after successful Ninja Forms submit.
 */
(function ($) {
	'use strict';
	...
	function removeExistingByFormId(formId) {
		if (!formId) return;
		$('.helpmate-ninja-forms-order-card[data-helpmate-form-id="' + String(formId) + '"]').remove();
	}
	...
	function renderCard($target, order, formId) {
		...
		var L = (window.helpmateNinjaFormsOrderStatus && window.helpmateNinjaFormsOrderStatus.labels) || {};
		removeExistingByFormId(formId);

		var card = el('div', 'helpmate-ninja-forms-order-card helpmate-cf7-order-status', {
			marginTop: '12px',
	...
	function renderFromPayloadNodes() {
		$('.helpmate-ninja-forms-order-status-payload').each(function () {
			var $node = $(this);
			...
			var $target = $node.closest('.nf-response-msg, .nf-response-msg-wrap');
			if (!$target.length) {
				$target = $node.closest('.nf-form-cont');
			}
			if (!$target.length && formId) {
				$target = $('#nf-form-' + String(formId) + '-cont');
			}
			if (!$target.length) {
				$target = $node.parent();
			}
