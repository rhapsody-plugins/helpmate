/**
 * Render Helpmate order details after successful Formidable submit
...
(function ($) {
	...
	function removeExistingByFormId(formId) {
		if (!formId) return;
		$('.helpmate-formidable-order-card[data-helpmate-form-id="' + String(formId) + '"]').remove();
	}
	...
	function renderCard($target, order, formId) {
		if (!order || typeof order !== 'object' || !$target || !$target.length) return;

		var L = (window.helpmateFormidableOrderStatus && window.helpmateFormidableOrderStatus.labels) || {};
		removeExistingByFormId(formId);

		var card = el('div', 'helpmate-formidable-order-card helpmate-cf7-order-status', {
			marginTop: '12px',
		...
		if (formId) card.setAttribute('data-helpmate-form-id', String(formId));
		...
		if (order.date) appendLabelValue(body, L.orderDate || 'Order date', String(order.date));
		if (order.trackingNumber) appendLabelValue(body, L.tracking || 'Tracking', String(order.trackingNumber));
		if (order.estimatedDelivery) appendLabelValue(body, L.estimatedDelivery || 'Estimated delivery', String(order.estimatedDelivery));
	...
	function renderFromPayloadNodes() {
		$('.helpmate-formidable-order-status-payload').each(function () {
			var $node = $(this);
			...
			var $target = $node.closest('.frm_message, .frm_ajax_loading');
			if (!$target.length) {
				$target = $node.closest('.frm-show-form');
			}
			if (!$target.length && formId) {
				$target = $('.frm-show-form[data-formid="' + String(formId) + '"]').first();
			}
			if (!$target.length) {
				$target = $node.parent();
			}
	...
	$(document).ready(function () {
		renderFromPayloadNodes();
	});

	$(document).ajaxComplete(function () {
		window.setTimeout(renderFromPayloadNodes, 50);
	});
})(jQuery);
