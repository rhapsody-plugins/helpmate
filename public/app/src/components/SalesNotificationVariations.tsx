...
function defaultActionPhrase(type: Order['type']): string {
  switch (type) {
    case 'sale':
      return __('just bought');
    case 'download':
      return __('just downloaded');
    default:
      return __('just reviewed');
  }
}

function compactActionVerb(type: Order['type']): string {
  switch (type) {
    case 'sale':
      return __('bought');
    case 'download':
    ...
    <div className="flex flex-col">
      <div className="text-xs">
        ...
        <span className="text-neutral-500">
          {defaultActionPhrase(order.type)}
        </span>
      </div>
      <Button
        variant="link"
      ...
      <div className="flex flex-row gap-2 justify-between items-center">
        <span className="flex gap-1 items-center text-xs text-[#0093E5]">
          <Bell className="w-3 h-3 fill-current" /> {__('By')}{' '}
          <a
            href={HelpmateURL}
            target="_blank"
    ...
    <Card className="flex flex-row relative -ml-8 items-center gap-2 pb-2 pt-6 !pr-16 w-full max-w-[300px] !pl-10 rounded-full shadow-lg">
      ...
      <div className="flex flex-col">
        <span className="flex absolute top-0 left-10 gap-1 items-center px-2 py-1 text-xs border-t-0 border border-[#9CB8FF] rounded-b-lg bg-[#ecf3ff]  w-fit text-[#0093E5]">
          <Bell className="w-3 h-3 fill-current text-[#0093E5]" /> {__('By')}{' '}
          <a
            href={HelpmateURL}
            target="_blank"
            rel="noopener noreferrer"
